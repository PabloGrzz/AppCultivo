...
fun ProductHighlights(
    ...
    Column(
        ...
        highlights.onEach { item ->
            Highlight(
                text = item.text ?: "", // Valor por defecto para textos nulos
                colors = HighlightDefaults.colors(item.type ?: ProductHighlightType.SECONDARY)
...
private fun Highlight(
    ...
    Surface(
        ...
        Box(
            ...
            Text(
                text = text,
                style = FrontEndPlantasTheme.typography.titleMedium.copy(fontWeight = FontWeight.Bold)
            )
        }
    }
}
...
private object HighlightDefaults {
    ...
    fun colors(type: ProductHighlightType): HighlightColors = when (type) {
        ProductHighlightType.SECONDARY -> HighlightColors(
            containerColor = FrontEndPlantasTheme.colors.actionSurface,
            contentColor = FrontEndPlantasTheme.colors.onActionSurface
        )

        ProductHighlightType.PRIMARY -> HighlightColors(
            containerColor = FrontEndPlantasTheme.colors.highlightSurface,
            contentColor = FrontEndPlantasTheme.colors.onHighlightSurface
        )
    }
}
